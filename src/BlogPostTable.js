...
// import { db, storage } from './firebase'; // Ensure this path is correct and the configuration is valid
...
//   //   const storageRef = ref(storage, `blogImages/${fileName}`); // Ensure path construction is correct
...
//     const fileName = encodeURIComponent(file.name); // Encode file name
//     const storageRef = ref(storage, `blogImages/${fileName}`);
    
//     try {
//       await uploadBytes(storageRef, file);
//       const imageUrl = await getDownloadURL(storageRef);
//       console.log('Uploaded Image URL:', imageUrl); // Log URL
//       return imageUrl;
//     } catch (error) {
...
//           const fileName = decodeURIComponent(postToDelete.imageUrl.split('/').pop().split('?')[0]); // Decode file name
//           const imageRef = ref(storage, `blogImages/${fileName}`);
          
//           console.log('Deleting image at:', imageRef.fullPath); // Log path
